...
`InMemorySignalStore` and `yubarta/ingestion/store.py` are removed. The storage port is now `IncidentStore`, whose implementation is Postgres-backed, and the seam is exercised in tests against a real database per the project's testing rule rather than against an in-memory fake.
