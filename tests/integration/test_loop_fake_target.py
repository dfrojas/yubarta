...
async def test_remediation_loop_fake_target(test_db: str) -> None:
    ...
    try:
        ...
        scanner = RemoteFileScanner(
            name="e2e-log", target=config.target, watch=config.watch[0]
        ...
        try:
            ...
            state.append_log(
                "[proxy_ajp:error] AH00957: AJP: Connection refused to 127.0.0.1:8009 (tomcat)"
            )
            ...
            async with sessions() as session:
                ...
                remediation_names = [
                    step.name for step in steps if step.kind == "remediation"
                ]
