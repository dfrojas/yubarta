...
def _health_server(state: FakeTargetState):  # type: ignore[no-untyped-def]
...
async def test_remediation_loop_fake_target(test_db):  # type: ignore[no-untyped-def]
    ...
    try:
        ...
        config = AppConfig(
            ...
            watch=[LogWatch(file="/var/log/apache2/error.log", match_any=["AH00957", "AJP", "Connection refused"])],
            diagnostics=["uptime", "free -h"],
            checks=[
                CommandCheck(run="sudo -n systemctl is-active --quiet tomcat9", expect_exit_code=0),
        ...
        try:
            ...
            async with sessions() as session:
                ...
                assert remediation_names == ["ensure-swap", "fix-restart-policy", "restart-tomcat"]
        finally:
            await scanner.stop()
