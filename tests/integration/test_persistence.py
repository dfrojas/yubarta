...
async def test_full_persistence_lifecycle(session_factory):  # type: ignore[no-untyped-def]
...
async def test_stdout_truncation(session_factory):  # type: ignore[no-untyped-def]
    async with session_factory() as session:
        repo = IncidentRepository(session)
